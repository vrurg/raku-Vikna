...
#                        note "TIMED OUT '$!message'";
        ...
        method pre-match(Event:D $ev) {
            return True unless $ev ~~ $!type || $!completed.status ~~ Planned;
            with $!wait-until {
                CATCH { default { note "AWAIT FUCKED UP WITH ", $_.WHICH; exit 1; } }
                $!completed.then: { $!wait-until.abort(False); };
                try await .promise;
#                note "wait({$!wait-until.name}) promise fulfilled";
                if .failed {
                    self!set-status: False;
                    return False;
                }
        ...
        method match(Event:D $ev) {
            return False unless $!completed.status ~~ Planned;
            with $!skip-until {
                unless .ready {
